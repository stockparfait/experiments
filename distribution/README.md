...
we can see that about 3/4 of non-empty the buckets contain at most one count, and most
...
$$
\mathrm{size}(\mathrm{bucket}[i+1]) = \gamma \cdot \mathrm{size}(\mathrm{bucket}[i])
$$

for some $\gamma > 1$. The formula obviously works
...
([config](assets/GOOG-vs-TSLA.json)), plotted as a $\log_{10}(\mathrm{p.d.f.})$ to see better
...
often with three degrees of freedom, the parameter I denote as $\alpha=3.0$.  The
justification often goes that, according to the [central limit theorem], a large
enough sum of T-distributed samples with $\alpha > 2$ becomes close to the normal
...
the samples within each stock to have $\mathrm{mean}=0.0$ and $\mathrm{MAD}=1.0$, where each
sample becomes:

$$
x' = \frac{x - E[X]}{\mathrm{MAD}[X]}
$$
...
comparison, I'm also throwing in the similarly normalized ($\mathrm{mean}=0.0$,
$\mathrm{MAD}=1.0$) T-distribution graph with $\alpha=3.0$
...
thickness of the tails of the sample distribution (in fact, $\alpha=2.55$ is a better fit).
...
database. It also now fits the $\alpha=3.0$ T-distribution a lot better, and has a
...
resembling Student's T with $\alpha\approx 3.0$ for liquid US stocks, i.e. with the average
...
in practice is basically _never_. However, a T-distributed sample with $\alpha=3$ can
